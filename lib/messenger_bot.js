 ...
 */
      ...
      if (!pageId) {
        if (!this.__associatedUpdate) {
          throw new Error(`can't perform the requested messengerProfileRequest: ${
            bodyOrQS
          } without a pageId option as you are using a botmaster-messenger outside with multiple pages outside of a middleware. Please specify pageId`);
        }
        pageId = this.__associatedUpdate.recipient.id;
      }
      pageToken = this.credentials.pages[pageId].pageToken;
  ...
  __getUserInfo(userId) {
    ...
    if (this.credentials.pages) {
      let pageId;
      if (!this.__associatedUpdate) {
        throw new Error('can\'t use getUserInfo when using botmaster-messenger with multiple pages outside of middleware. use _getUserInfoFromPage instead');
      } else {
        pageId = this.__associatedUpdate.recipient.id;
      }
      pageToken = this.credentials.pages[pageId].pageToken;
